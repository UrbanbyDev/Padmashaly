...
public class AdapterUserNews extends RecyclerView.Adapter {
    ...
    private Boolean isAdLoaded = false, isUser;
    private List<NativeAd> mNativeAdsAdmob = new ArrayList<>();

    public AdapterUserNews(Context context, ArrayList<ItemNews> arrayList, Boolean isUser) {
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        TextView textView_heading, textView_views, textView_cat,approval_status;
        ImageView imageView_fav, imageView_play, iv_approved;
        ...
        MyViewHolder(View view) {
            ...
            iv_approved = view.findViewById(R.id.iv_home_news_approved);

            approval_status=view.findViewById(R.id.approval_status);

            TooltipCompat.setTooltipText(iv_approved, context.getString(R.string.approved));
        }
    }
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MyViewHolder) {
            ...
            if (isUser && arrayList.get(position).getIsApproved()) {
                ((MyViewHolder) holder).iv_approved.setVisibility(View.VISIBLE);
                ((MyViewHolder) holder).approval_status.setText("Approved");
                ((MyViewHolder) holder).approval_status.setTextColor(Color.parseColor("#00FF00"));

            } else {
                ((MyViewHolder) holder).iv_approved.setVisibility(View.GONE);
                ((MyViewHolder) holder).approval_status.setText("Pending");
                ((MyViewHolder) holder).approval_status.setTextColor(Color.parseColor("#FF0000"));
            }
            ...
            methods.setFavImage(arrayList.get(holder.getAbsoluteAdapterPosition()).getIsFav(), ((MyViewHolder) holder).imageView_fav);
            methods.setLikeImage(arrayList.get(holder.getAbsoluteAdapterPosition()).getIsLike(), ((MyViewHolder) holder).fab_like);
            ...
            ((MyViewHolder) holder).textView_views.setText(arrayList.get(position).getTotalViews());

            String imageurl = "null";
            imageurl = methods.getImageThumbSize(arrayList.get(holder.getAbsoluteAdapterPosition()).getImageThumb(), "header");
            Picasso.get()
                    .load(imageurl)
...
//                    methods.showInterAd(holder.getAbsoluteAdapterPosition(), "");
                    ...
                    MySingleton.getInstance().getNews().clear();
                    MySingleton.getInstance().getNews().addAll(arrayList);
