...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public String getRecentNewsIDs(String limit) {
        ...
        Cursor cursor = db.query(TABLE_RECENT, new String[]{TAG_NID}, null, null, null, null, TAG_ID + " DESC", limit);

        if (cursor != null && cursor.getCount() > 0) {
            cursor.moveToFirst();
            newsIDs = cursor.getString(cursor.getColumnIndex(TAG_NID));
            cursor.moveToNext();
            for (int i = 1; i < cursor.getCount(); i++) {
                newsIDs = newsIDs + "," + cursor.getString(cursor.getColumnIndex(TAG_NID));
